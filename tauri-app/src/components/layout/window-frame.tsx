...
export function WindowFrame({
    ...
    const onMove = (e: MouseEvent) => {
      ...
      if (resizeRef.current) {
        ...
        if (r.dir?.includes("n")) {
          ...
          ny = Math.max(0, r.origY + (r.origH - newH));
          nh = newH;
        }
  ...
  const toggleMaximize = () => {
    if (maximized) {
      setPos(preMaxState.pos); setSize(preMaxState.size); setMaximized(false);
    } else {
      setPreMaxState({ pos, size });
      setPos({ x: 0, y: 0 }); setSize({ w: window.innerWidth, h: window.innerHeight - 40 }); setMaximized(true);
    }
  };

  if (minimized) return null;

  // Edge/corner handle style
  const handle = (dir: ResizeDir, cursor: string, style: React.CSSProperties) => (
    <div
  ...
  return (
    <div
      ...
      onMouseDown={onFocus}
    >
      {/* Resize handles — invisible 5px strips on edges and 10px squares on corners */}
      {!maximized && (<>
        {handle("n", "ns-resize", { top: -3, left: 10, right: 10, height: 6 })}
        {handle("s", "ns-resize", { bottom: -3, left: 10, right: 10, height: 6 })}
        {handle("w", "ew-resize", { left: -3, top: 10, bottom: 10, width: 6 })}
        {handle("e", "ew-resize", { right: -3, top: 10, bottom: 10, width: 6 })}
        {handle("nw", "nwse-resize", { top: -4, left: -4, width: 12, height: 12 })}
        {handle("ne", "nesw-resize", { top: -4, right: -4, width: 12, height: 12 })}
        {handle("sw", "nesw-resize", { bottom: -4, left: -4, width: 12, height: 12 })}
        {handle("se", "nwse-resize", { bottom: -4, right: -4, width: 12, height: 12 })}
      </>)}
