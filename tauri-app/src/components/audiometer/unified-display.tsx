  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-px text-[10px]">
        ...
        <div className="flex justify-between">
          <span className="ls-text-muted">Salida</span>
          <span className={cn("font-semibold", ch.output === "right" ? "text-red-400/70" : "text-blue-400/70")}>
            {ch.output === "right" ? "Derecha" : "Izquierda"}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="ls-text-muted">Estím</span>
          <span className="font-semibold ls-text2">{stimL[ch.stimulus]}</span>
        </div>
        <div className="flex justify-between">
          <span className="ls-text-muted">Modo</span>
          <span className="font-semibold ls-text2">{modeL[ch.toneMode]}</span>
        </div>
        <div className="flex justify-between">
          <span className="ls-text-muted">Pasos</span>
          <span className="font-semibold ls-text2">{ch.step} dB</span>
        </div>
        <div className="flex justify-between">
          <span className="ls-text-muted">Revers</span>
          <span className={cn("font-semibold", ch.reversed ? "text-amber-400/70" : "ls-text-muted")}>
...
export function UnifiedDisplay({ ch0, ch1, frequency, testMode, time, logoText }: UnifiedDisplayProps) {
  return (
    <div className="flex rounded border ls-border bg-gradient-to-b from-[#080a0d] to-[#0d1014]">
      {/* CH0 */}
      <div className="flex-1 border-r ls-border px-3 py-2">
      ...
      {/* CH1 */}
      <div className="flex-1 border-l ls-border px-3 py-2">
