...
- [ ] `response.py:response_sdt_w_mkg` (mano levantada del SDT) exige el
...
- [ ] `Audiometer._mkg_on` exige `"Invertido"` en el canal del ruido: en
...
- [ ] "Sin respuesta" (130) en el cliente: `src/audiometria/` y `src/abr/`
      ...
      130 dB
- [ ] La tonal no distingue "no se midió" de "no respondió": las dos se
...
- [ ] Voces de logoaudiometría: `ListWords.la_super` fuerza `feme1` porque
...
- [ ] El PDF del caso siempre dibuja ABR/VEMP con la normativa por defecto:
      no consulta la del curso (`AppConfig`, `normative_data.abr`).
      § Los trazos del PDF no son el generador
- [ ] Ningún test compara `CaseWaveforms::CLICK_BASE` y
...
- [ ] El informe de otoscopia no se guarda local: sin conexión, al cerrar la
...
- [ ] **Dilema de enmascaramiento.** `_masking_calc` hace `sorted` al rango
...
- [ ] **Efecto oclusivo: fuente de la tabla** `[15,15,15,10,0…]`. Preguntarle
...
- [ ] **CE del ruido enmascarante.** Confirmar `CE_TONAL` con la docente y si
      ...
      enmascarante y CE
- [ ] **Curva sombra en logoaudiometría**: preguntarle a la docente qué
      discriminación espera en una anacusia unilateral. § Curva sombra en
      logoaudiometría
- [ ] **Pediatría.** ¿Se implementan VRA y juego condicionado, o el caso
...
- [ ] **¿Los pacientes están congelados o envejecen?** Sugerencia anotada:
...
- [ ] **Gradiente del timpanograma**: acá 1 = curva ancha, la de Brooks es al
      revés (`1 - esto`). Dar vuelta la resta toca `Z_225._calc_gradient`,
      `Z.move` y `CaseCharts::gradienteDe`. § Rangos de compliance y presión
      por letra
- [ ] **Razón V/I del ABR** (piso normal pasó a 1.0): `v_i_factor` (default
...
- [ ] **Efecto de oclusión en el ABR óseo** (postergado con aval del docente):
...
- [ ] **Marcado automático del ECochG**: ¿lo ve el alumno o solo el docente?
      Hoy los dos. § Marcado automático
- [ ] **Aviso de reloj desfasado en la app**: `BackendClient.clock_skew()`
      existe y está testeada, pero nadie la llama. ¿Al iniciar sesión o en el
      cartel de "sin conexión"? § El reloj: una sola zona
- [ ] **F5 de cursos (ciclo de vida)**: `code`/`term`/fechas, clonar curso,
...
### Build y arranque
- [ ] Achicar el build (~90 MB, solo `LabSim.spec`): tema GTK, QtQuick/Qml,
...
- [ ] Arranque ~2 s más rápido: importar `scipy.signal` dentro de las
...
§ Otoscopia: derivación + aprobación docente + fase por alumno
- [ ] Derivación del alumno al cerrar la atención (`attendance_action.php`,
      `atendido`), separada de la nota general.
- [ ] Bandeja docente para aprobar/rechazar derivaciones por alumno-paciente.
- [ ] Tabla de progreso alumno-paciente (fase aprobada de cada uno).
- [ ] Motor de selección de fase (clamp a la última), también en la app,
      mostrando el texto libre de la fase.
- [ ] Aviso al alumno al aprobar/rechazar (`inbox_messages`).
- [ ] Historial de derivaciones (quién, cuándo, quién aprobó).
- [ ] Reglas de reintento tras un rechazo.
- [ ] Otoscopía neumática y checklist de membrana sin localización (como
...
- [ ] Definir la key (`audiometry_params`, un solo blob).
- [ ] Juntar los literales en un `audiometria/params.py` que consulte
      `app_config_store` con el valor actual como default.
- [ ] Entrada en `src/CourseParams.php`, con límites que impidan valores
      imposibles y ayuda mecánica.
- [ ] Confirmar qué pasa con los casos armados cuando un curso cambia un
      parámetro.
- [ ] Unificar `GAP_MAX_DB` (hoy en cliente y backend).
- [ ] CE dependiente de la frecuencia. § Tipo de ruido enmascarante y CE
- [ ] Atenuación interaural del habla por bandas en vez del escalar 45.
...
- [ ] Reflejo presente con gap (dehiscencia de canal superior, acueducto
      vestibular dilatado).
- [ ] Ósea supranormal (tercera ventana, Pendred, Mondini).
- [ ] Eje temporal / fluctuación (Ménière, CMV, NIHL, salicilatos, tuba
      abierta). El cambio más grande.
- [ ] Asimetría declarada por el cuadro (autoinmune, CMV).
- [ ] Otoscopia ligada al cuadro (OMA, colesteatoma, perforación, glomus,
      atresia).
- [ ] Timpanograma "no registrable" en `Z_OPTIONS`.
- [ ] Eje no orgánico (inconsistencias, SRT vs PTA, Stenger).
- [ ] Procesamiento auditivo central (pruebas dicóticas).
- [ ] Hiperacusia / misofonía como eje del perfil.

### ABR
- [ ] Conectar al modelo los 17 parámetros de `UNCONNECTED_SETTINGS`
      (envolvente del burst, ventana, notch, suavizado…): leerlo, testearlo y
      sacarlo de la lista. § Parámetros avanzados del ABR
- [ ] Normativa: el tramo de 1 a 3 años cae en `child` (valores de adulto);
      burst de 1-4 kHz sigue derivado. § ABR: normativa anclada en bibliografía
- [ ] Ficha PDF: tabla de umbrales por vía ósea (`umbral_por_estimulo_oseo`
      ya se calcula). § ABR: catálogo real de estímulos
- [ ] Ficha de estudio: mostrar el ECochG. § Electrococleografía: el motor

### EOA
- [ ] Parámetros propios del TEOAE en la ficha (reproducibilidad,
...
### VEMP
- [ ] Umbral y sintonía editables en la ficha (`_vemp.php`,
      `CaseForm::parseVemp`) para armar un hidrops a mano. § VEMP v2
- [ ] Impedancia de electrodos (un campo más de `Ajustes`). § VEMP v2

### Impedanciometría
- [ ] Compliance y presión exactas en la ficha: sortearlas en el backend al
      crear el caso y que la app las lea. § La compliance y la presión del
      timpanograma no se pueden anticipar

### Menores
- [ ] Los `print()` de ABR, EOA y VEMP son de error; si algún día imprimen
