...
class Curva:
    def __init__(self):
        self.inicio = [(0,0),(1,0)]
        self.I = [(1.6,0.1),(1.8,-0.2)]
        self.II = [(2.6,0.1),(3,-0.03)]
        self.III = [(3.6,0.4),(4,-0.15)]
        self.IV = [(4.5,-0.1),(5,-0.15)]
        self.V = [(5.5,0.13),(6,-0.4)]
        self.final = [(7,0),(12,0)]

    def modificar_latencia(self, curva, latencia):
        if hasattr(self, curva):
            diferencia = latencia - getattr(self, curva)[0][0]
            
            if curva == "I":
                self.I = [(p[0] + diferencia, p[1]) for p in self.I]
                self.II = [(p[0] + diferencia, p[1]) for p in self.II]
                self.III = [(p[0] + diferencia + 2.0, p[1]) for p in self.III]
                self.IV = [(p[0] + diferencia + 2.0, p[1]) for p in self.IV]
                self.V = [(p[0] + diferencia + 4.0, p[1]) for p in self.V]
                
            elif curva == "III":
                self.I = [(p[0] + diferencia - 2.0, p[1]) for p in self.I]
                self.II = [(p[0] + diferencia - 2.0, p[1]) for p in self.II]
                self.III = [(p[0] + diferencia, p[1]) for p in self.III]
                self.IV = [(p[0] + diferencia, p[1]) for p in self.IV]
                self.V = [(p[0] + diferencia + 2.0, p[1]) for p in self.V]
                
            elif curva == "V":
                self.I = [(p[0] + diferencia - 4.0, p[1]) for p in self.I]
                self.II = [(p[0] + diferencia - 4.0, p[1]) for p in self.II]
                self.III = [(p[0] + diferencia - 2.0, p[1]) for p in self.III]
                self.IV = [(p[0] + diferencia - 2.0, p[1]) for p in self.IV]
                self.V = [(p[0] + diferencia, p[1]) for p in self.V]

            # Verificación para evitar tiempos negativos
            if self.I[0][0] < 0:
                correcion = abs(self.I[0][0])
                self.I = [(p[0] + correcion, p[1]) for p in self.I]
                self.II = [(p[0] + correcion, p[1]) for p in self.II]
                self.III = [(p[0] + correcion, p[1]) for p in self.III]
                self.IV = [(p[0] + correcion, p[1]) for p in self.IV]
                self.V = [(p[0] + correcion, p[1]) for p in self.V]
    ...
    def obtener_curva_completa(self):
        return self.inicio + self.I + self.II + self.III + self.IV + self.V + self.final

# Ejemplo de uso:
c = Curva()
c.modificar_latencia("III", 5.0)  # Cambiar latencia de curva III. Las otras curvas también se ajustarán según los interpeaks.
