...
class SoundCreate:
    def __init__(self, duration=1.0, sample_rate=44100, channel="both"):
        #self.appctxt = AppContextManager.get_context()
        format = QAudioFormat()
    ...
    def get_file_path(self, noise_type):
        try:
            return self.appctxt.get_resource(f"{noise_type}_noise.dat")
        except FileNotFoundError:
            return False

    def save_to_file(self, data, noise_type):
        file_path = self.get_file_path(noise_type)
        with open(file_path, 'wb') as f:
            f.write(data)

    def load_from_file(self, noise_type):
        file_path = self.get_file_path(noise_type)
        with open(file_path, 'rb') as f:
            return f.read()

    def play(self, noise_type="white", frecuency=1000):
        #file_path = self.get_file_path(noise_type)
        ...
        if noise_type == "white":
            ...
            data = self.generate_nbn(center_freq=frecuency)
        elif noise_type == "tone":
            data = self.generate_tone(frequency=frecuency)
        else:
...
if __name__ == "__main__":
    ...
    player = SoundCreate(duration=5, channel="left")  # Sonará solo en el canal izquierdo
