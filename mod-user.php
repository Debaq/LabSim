...
$URI_code = "mod-user.php?";
$URI_codeTest = strpos($URI_into, $URI_code);
$newQuestionLink = codifica_urlget("nameArea=Estudiantes&idArea=0&view=false&idQ=0");
...
if ($URI_codeTest) {
    ...
    if($btn == "Desactivar"){
        $Q_UPDATE = mysqli_query($mysqli, "UPDATE users SET Activo='0'WHERE id_user_O='$idQ'");
        mysqli_close($mysqli);
        header("location:index.php?$newQuestionLink");
    }
    if($btn == "Activar"){
        $Q_UPDATE = mysqli_query($mysqli, "UPDATE users SET Activo='1'WHERE id_user_O='$idQ'");
        mysqli_close($mysqli);
        header("location:index.php?$newQuestionLink");
    }
    if($btn == "Eliminar"){
        $Q_UPDATE = mysqli_query($mysqli, "DELETE FROM users WHERE id_user_O='$idQ'");
        mysqli_close($mysqli);
        header("location:index.php?$newQuestionLink");
    }

    if($btn == "Descargar"){
        $getQuestion = mysqli_query($mysqli, "SELECT * FROM users WHERE id_user_O = '$idQ'");
